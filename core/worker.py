...
from core.db_session import set_tenant_context
...
async def process_job(agent: Agent, job: Job) -> dict[str, Any]:
    """Dispatch a job to the correct agent method. Returns result dict."""

    if job.job_type == "shadow_audit":
        result = await agent.run_shadow_audit(job.input_payload)
    elif job.job_type == "prior_auth":
        result = await agent.run_prior_auth(job.input_payload)
    else:
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                result = await process_job(agent, job)
